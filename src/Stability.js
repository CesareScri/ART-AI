...
const Stability = () => {
  ...
  const handleMessageSubmit = async (newMsg) => {
    ...
    try {
      const response = await fetch('http://localhost:3001/v2/predictions', options);
      if (!response.ok) {
  ...
  const handleCreateIMG = (answer, newMsg, prevQuestion) => {
    ...
    const newIntervalId = setInterval(async () => {
      try {
        const response = await fetch(`http://localhost:3001/v2/predictions/${answer}`);
        if (!response.ok) {
          throw new Error('Request failed');
        }
  ...
  return (
    <div className='r-cnt'>
    <p className='model-info'>A latent text-to-image diffusion model capable of generating photo-realistic images given any text input</p>
...
const Container = ({ imageUrl, loading, status, setMessages, handleSubmit, handleMessageSubmit, handleCreateIMG, answer }) => {
  ...
  const handleDownload = () => {
    ...
    const Countdown = () => {
      ...
      return <img className='img-loading' src={Gif} width='40px'></img>;
    };
      ...
      {loading ? (
        ...
        <div className='img-container'>
          ...
          <button className='btn-reg' onClick={handleRegenerate} disabled={regenerating}>
            {!regenerating ? (<p>Regenerate</p>) : <Countdown /> }
          </button>
